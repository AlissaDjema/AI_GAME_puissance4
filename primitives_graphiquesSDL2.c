...
// choisir la bonne version en fonction de la SDL présente sur votre ordinateur
...
void initialise_fenetre(char* titre)
{
	...
	if( SDL_Init( SDL_INIT_VIDEO ) < 0 )
	    {
	        printf( "SDL ne peut pas etre demarree  SDL_Error: %s\n", SDL_GetError() );
	    }
	    else
	    {
	        //Create window
	        ...
	        if( fenetre == NULL )
	        {
	            printf( "Impossible de creer la fenetre SDL_Error: %s\n", SDL_GetError() );
	        }
	        else
	        {
	            //ecran est la surface de la fenetre
	           ecran= SDL_GetWindowSurface( fenetre );

	            //On remplit ecran en noir
...
SDL_Surface* charge_image( const char* chemin_image)
{
	...
	{
		surface= SDL_ConvertSurface(tampon,ecran->format,0);
	}
	if ( surface == NULL )
	{
...
void ligne(int xi,int yi,int xf,int yf,Uint8 r,Uint8 v, Uint8 c)
{
  double x,y ;
  double a,b ;
  ...
  if((yf-yi)<(xf-xi))
  {
  	for ( x = xi ; x <= xf ; x++ )
  	{
    		...
    		if(y>yi+1./2)
    		{
    			colorie_pixel_surface(ecran,x, yi+1 ,r,v,c);
		   }
		else if(yi+1./2>y && y>yi-1/.2)
		{
			colorie_pixel_surface(ecran,x,yi,r,v,c);
		}
		else
		{
			colorie_pixel_surface(ecran,x,yi-1,r,v,c);
  ...
  else
  {
    ...
    x =(int) ((y-b)/a) ;
        ...
        if(x>xi+1/2)
    	{
    		colorie_pixel_surface(ecran,xi+1,y,r,v,c);
		}
		else if(xi+1/2>x && x>xi-1/2)
		{
			colorie_pixel_surface(ecran,xi,y,r,v,c);
		}
		else
		{
			colorie_pixel_surface(ecran,xi-1,y,r,v,c);
